...
import type { ClassificationRuleResponse, SiteDetailResponse, SiteEntryResponse, SiteListItemResponse, SiteMergePreviewResponse, SiteSplitPreviewResponse, SiteSplitRequest, SiteUpdateRequest } from '../lib/api/types'

type SiteView = 'all' | 'pinned' | 'recent' | 'review'
const labels: Record<SiteView, string> = { all: '全部网站', pinned: '已置顶', recent: '最近收藏', review: '待审核' }
...
export function SitesView({ client, onToast, initialSiteId, collapsed, onView, onNavigate, navigationOpen, onCloseNavigation, capabilityLease }: SitesViewProps) {
	...
	useEffect(() => {
		...
		const apply = async (request: ReturnType<ReaderClient['updateSite']>) => {
	...
	const removeSite = async () => {
		...
		const mergeSite = async (request: Parameters<ReaderClient['executeSiteMerge']>[0]) => {
		...
		const splitSite = async (siteID: string, request: Parameters<ReaderClient['executeSiteSplit']>[1]) => {
		...
		return <section className={'sites-view' + (active ? ' has-detail' : '')}>
			<aside className={'sites-sidebar' + (collapsed ? ' collapsed' : '')} id="primary-navigation" aria-label="网站导航">
				...
				<div className="sites-filter-group">
					...
					{policy.siteAdvanced && <button type="button" className="sites-nav sites-rules-nav" onClick={() => { if (canManage()) setManagingRules(true) }}>分类规则</button>}
				</div>
			</aside>
		...
		{policy.siteWrite && pendingReadingEntry && <MoveEntryToReadingDialog entry={pendingReadingEntry} busy={convertingReading} onClose={() => { if (!convertingReading) setPendingReadingEntry(null) }} onConfirm={() => void confirmMoveEntryToReading()} />}
		{policy.siteAdvanced && managingRules && <ClassificationRulesDialog capabilityLease={capabilityLease} client={client} onClose={() => setManagingRules(false)} onToast={onToast} />}
...
function ClassificationRulesDialog({ client, capabilityLease, onClose, onToast }: { client: ReaderClient; capabilityLease: ReaderCapabilityLease; onClose: () => void; onToast: (message: string, icon?: import('./Icon').IconName) => void }) {
	const [rules, setRules] = useState<ClassificationRuleResponse[]>([])
	const [loading, setLoading] = useState(true)
	const [host, setHost] = useState('')
	const [target, setTarget] = useState<'reading' | 'site'>('site')
	useEffect(() => { if (!capabilityLease.isCurrent('siteAdvanced')) return; void client.getClassificationRules().then((result) => { if (!client.isIdentityCurrent() || !capabilityLease.isCurrent('siteAdvanced')) return; if (result.ok) setRules(result.data); else onToast(`加载分类规则失败：${result.error.message}`, 'alert'); setLoading(false) }) }, [capabilityLease, client, onToast])
	const create = async (event: FormEvent) => { event.preventDefault(); if (!capabilityLease.isCurrent('siteAdvanced')) return; const result = await client.createClassificationRule({ host, target_kind: target }); if (!client.isIdentityCurrent() || !capabilityLease.isCurrent('siteAdvanced')) return; if (!result.ok) { onToast(`创建规则失败：${result.error.message}`, 'alert'); return } setRules((current) => [...current, result.data]); setHost(''); onToast('分类规则已创建', 'check') }
	const toggle = async (rule: ClassificationRuleResponse) => { if (!capabilityLease.isCurrent('siteAdvanced')) return; const result = await client.updateClassificationRule(rule.id, rule.revision, { enabled: !rule.enabled }); if (!client.isIdentityCurrent() || !capabilityLease.isCurrent('siteAdvanced')) return; if (!result.ok) { onToast(`更新规则失败：${result.error.message}`, 'alert'); return } setRules((current) => current.map((item) => item.id === rule.id ? result.data : item)) }
	const remove = async (rule: ClassificationRuleResponse) => { if (!capabilityLease.isCurrent('siteAdvanced')) return; const result = await client.deleteClassificationRule(rule.id, rule.revision); if (!client.isIdentityCurrent() || !capabilityLease.isCurrent('siteAdvanced')) return; if (!result.ok) { onToast(`删除规则失败：${result.error.message}`, 'alert'); return } setRules((current) => current.filter((item) => item.id !== rule.id)); onToast('分类规则已删除', 'trash') }
	return <div className="site-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="site-dialog" role="dialog" aria-modal="true" aria-labelledby="classification-rules-title" onMouseDown={(event) => event.stopPropagation()}><header><h2 id="classification-rules-title">分类规则</h2><button type="button" className="tb-btn" onClick={onClose} title="关闭" aria-label="关闭"><Icon name="close" size={15} /></button></header><p className="classification-rules-note">规则只影响之后的自动采集，不会移动已有收藏。</p>{loading ? <div className="sites-empty">正在加载规则</div> : <ul className="classification-rule-list">{rules.map((rule) => <li key={rule.id}><span><strong>{rule.host}{rule.path_prefix ?? ''}</strong><small>{rule.target_kind === 'reading' ? '归入阅读' : '归入网站'}{rule.identity_adapter ? ` · ${rule.identity_adapter}` : ''}</small></span><button type="button" onClick={() => void toggle(rule)}>{rule.enabled ? '已启用' : '已停用'}</button><button type="button" className="tb-btn danger" title="删除规则" aria-label={`删除 ${rule.host} 规则`} onClick={() => void remove(rule)}><Icon name="trash" size={15} /></button></li>)}</ul>}<form onSubmit={(event) => void create(event)}><label>域名<input value={host} onChange={(event) => setHost(event.target.value)} placeholder="example.com" required maxLength={253} /></label><label>归类<select value={target} onChange={(event) => setTarget(event.target.value as 'reading' | 'site')}><option value="site">网站</option><option value="reading">阅读</option></select></label><footer><button type="button" onClick={onClose}>关闭</button><button type="submit">创建规则</button></footer></form></section></div>
}
...
function SiteMergeDialog({ client, capabilityLease, target, onClose, onMerge, onToast }: { client: ReaderClient; capabilityLease: ReaderCapabilityLease; target: SiteDetailResponse; onClose: () => void; onMerge: (request: Parameters<ReaderClient['executeSiteMerge']>[0]) => Promise<boolean>; onToast: (message: string, icon?: import('./Icon').IconName) => void }) {
...
function SiteSplitDialog({ client, capabilityLease, site, onClose, onSplit, onToast }: { client: ReaderClient; capabilityLease: ReaderCapabilityLease; site: SiteDetailResponse; onClose: () => void; onSplit: (siteID: string, request: SiteSplitRequest) => Promise<SiteSplitOutcome>; onToast: (message: string, icon?: import('./Icon').IconName) => void }) {
	...
	return <div className="site-dialog-backdrop" role="presentation" onMouseDown={() => { if (!busy) onClose() }}><form className="site-dialog" role="dialog" aria-modal="true" aria-labelledby="site-split-title" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void makePreview() }}><header><h2 id="site-split-title">拆分网站</h2><button type="button" className="tb-btn" onClick={onClose} disabled={busy} title="关闭" aria-label="关闭"><Icon name="close" size={15} /></button></header><p>选择要移动的入口，并为新网站单独填写资料。</p><ul className="classification-rule-list">{site.entries.map((entry) => <li key={entry.id}><label><input type="checkbox" checked={selected.includes(entry.id)} onChange={() => toggle(entry.id)} /> <strong>{entry.name || entry.url}</strong><small>{entry.url}</small></label></li>)}</ul><label>新网站名称<input value={name} onChange={(event) => { setName(event.target.value); invalidatePreview() }} required maxLength={256} /></label><label>简介<textarea value={intro} onChange={(event) => { setIntro(event.target.value); invalidatePreview() }} maxLength={1000} /></label><label>主页<input type="url" value={homepage} onChange={(event) => { setHomepage(event.target.value); invalidatePreview() }} maxLength={2048} /></label><label>图标<input type="url" value={icon} onChange={(event) => { setIcon(event.target.value); invalidatePreview() }} maxLength={2048} /></label><label>用户备注<textarea value={note} onChange={(event) => { setNote(event.target.value); invalidatePreview() }} maxLength={10000} /></label><label>新网站主入口<select value={primaryEntryID} onChange={(event) => { setPrimaryEntryID(event.target.value); invalidatePreview() }} required><option value="">请选择</option>{site.entries.filter((entry) => selected.includes(entry.id)).map((entry) => <option key={entry.id} value={entry.id}>{entry.name || entry.url}</option>)}</select></label>{identityOptions.length > 0 && <label>站点身份<select value={identityKey} onChange={(event) => { setIdentityKey(event.target.value); invalidatePreview() }}><option value="">全部留在原网站</option>{eligibleIdentities.map((identity) => <option key={identity.identity_key} value={identity.identity_key}>转移 {identity.identity_key}</option>)}</select></label>}{identityOptions.length > 0 && eligibleIdentities.length === 0 && <p>被移动入口没有可转移的站点身份。</p>}{validationError && <p role="alert">{validationError}</p>}{preview && <section aria-label="拆分预览"><h3>最终拆分</h3><p>新网站：{preview.payload.name}</p><p>简介：{preview.payload.intro || '空'}</p><p>主页：{preview.payload.homepage_url || '空'}</p><p>图标：{preview.payload.icon_url || '空'}</p><p>用户备注：{preview.payload.user_note || '空'}</p><p>移动入口：{preview.entries.map((entry) => entry.name || entry.url).join('、')}</p><ul>{preview.identities.map((identity) => <li key={identity.identity_key}>{identity.identity_key}：{identity.owner === 'new_site' ? '新网站' : '原网站'}</li>)}</ul></section>}<footer><button type="button" onClick={onClose} disabled={busy}>取消</button>{preview ? <button type="button" onClick={() => void execute()} disabled={busy}>{busy ? '拆分中' : '确认拆分'}</button> : <button type="submit" disabled={busy}>{busy ? '加载中' : hasPreviewed ? '更新预览' : '预览拆分'}</button>}</footer></form></div>
}

function SiteProfileDialog({ site, onClose, onSave }: { site: SiteDetailResponse; onClose: () => void; onSave: (patch: SiteUpdateRequest) => Promise<boolean> }) { const [name, setName] = useState(site.name); const [intro, setIntro] = useState(site.intro); const [homepage, setHomepage] = useState(site.homepage_url ?? ''); const [note, setNote] = useState(site.user_note); const [tags, setTags] = useState(site.tags.join(', ')); const [saving, setSaving] = useState(false); const submit = async (event: FormEvent) => { event.preventDefault(); const nextTags = tags.split(',').map((tag) => tag.trim()).filter(Boolean); const existing = new Map(site.tags.map((tag) => [tag.toLocaleLowerCase(), tag])); const next = new Map(nextTags.map((tag) => [tag.toLocaleLowerCase(), tag])); const add = [...next].filter(([key]) => !existing.has(key)).map(([, tag]) => tag); const remove = [...existing].filter(([key]) => !next.has(key)).map(([, tag]) => tag); const patch: SiteUpdateRequest = { name, intro, user_note: note }; if (homepage.trim()) patch.homepage_url = homepage.trim(); if (add.length || remove.length) patch.tags = { add, remove }; setSaving(true); const saved = await onSave(patch); setSaving(false); if (saved) onClose() }; return <div className="site-dialog-backdrop"><form className="site-dialog" onSubmit={(event) => void submit(event)}><header><h2>编辑网站资料</h2><button type="button" className="tb-btn" onClick={onClose} title="关闭" aria-label="关闭"><Icon name="close" size={15} /></button></header><label>名称<input value={name} onChange={(event) => setName(event.target.value)} required maxLength={256} /></label><label>简介<textarea value={intro} onChange={(event) => setIntro(event.target.value)} maxLength={1000} /></label><label>主页<input type="url" value={homepage} onChange={(event) => setHomepage(event.target.value)} /></label><label>标签<input value={tags} onChange={(event) => setTags(event.target.value)} /></label><label>用户备注<textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={10000} /></label><footer><button type="button" onClick={onClose}>取消</button><button type="submit" disabled={saving}>{saving ? '保存中' : '保存'}</button></footer></form></div> }

function SiteEntryDialog({ entry, onClose, onSave }: { entry: SiteEntryResponse; onClose: () => void; onSave: (patch: { name: string; purpose: string }) => Promise<boolean> }) { const [name, setName] = useState(entry.name); const [purpose, setPurpose] = useState(entry.purpose); const [saving, setSaving] = useState(false); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); const saved = await onSave({ name, purpose }); setSaving(false); if (saved) onClose() }; return <div className="site-dialog-backdrop"><form className="site-dialog compact" onSubmit={(event) => void submit(event)}><header><h2>编辑入口</h2><button type="button" className="tb-btn" onClick={onClose} title="关闭" aria-label="关闭"><Icon name="close" size={15} /></button></header><label>名称<input value={name} onChange={(event) => setName(event.target.value)} required maxLength={256} /></label><label>用途<textarea value={purpose} onChange={(event) => setPurpose(event.target.value)} maxLength={1000} /></label><footer><button type="button" onClick={onClose}>取消</button><button type="submit" disabled={saving}>{saving ? '保存中' : '保存'}</button></footer></form></div> }
...
function MoveEntryToReadingDialog({ entry, busy, onClose, onConfirm }: { entry: SiteEntryResponse; busy: boolean; onClose: () => void; onConfirm: () => void }) {
	...
	return <div className="site-dialog-backdrop" role="presentation" onMouseDown={() => { if (!busy) onClose() }}><section className="site-dialog compact" role="dialog" aria-modal="true" aria-labelledby="move-entry-to-reading-title" onMouseDown={(event) => event.stopPropagation()} onKeyDown={onKeyDown}><header><h2 id="move-entry-to-reading-title">移到阅读</h2><button type="button" className="tb-btn" onClick={onClose} disabled={busy} title="关闭" aria-label="关闭"><Icon name="close" size={15} /></button></header><p>入口“{entry.name || entry.url}”将离开网站库，并重新抓取和解析为阅读内容。</p><footer><button ref={cancelRef} type="button" onClick={onClose} disabled={busy}>取消</button><button ref={confirmRef} type="button" onClick={onConfirm} disabled={busy}>{busy ? '转换中' : '确认转换'}</button></footer></section></div>
}
