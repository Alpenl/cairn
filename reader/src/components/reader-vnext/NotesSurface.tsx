...
export function NotesSurface({ client, lease, capabilityLease, onNavigate, initialNoteID, onDraftDirtyChange, onPendingPersistenceChange, onPrepareToLeaveChange, onCreateNote, creatingNote = false, annotationsEnabled = false, aiEnabled = false, trashEnabled = false }: NotesSurfaceProps) {
  ...
  return (
    <SurfaceShell
      ...
      actions={<><NoteWorkspaceTabs active="notes" onNavigate={onNavigate} capabilityPolicy={capabilityLease.policy} /><div className="rvx-segmented" aria-label="笔记视图"><button type="button" className={view === 'active' ? 'active' : ''} onClick={() => void switchView('active')}>进行中</button>{trashEnabled && <button type="button" className={view === 'trash' ? 'active' : ''} onClick={() => void switchView('trash')}>回收站{trashCount === null ? '' : ` (${trashCount})`}</button>}</div>{view === 'active' && onCreateNote && <button className="rvx-icon-button" type="button" title="新建笔记" aria-label="新建笔记" disabled={saving || creatingNote} onClick={onCreateNote}><Icon name="plus" size={16} /></button>}</>}
    >
      {view === 'trash' ? <>
