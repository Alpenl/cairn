...
export function AddSubscriptionDialog({
  ...
  // 关闭规则原样保留：backdrop 与 Escape 都可关闭，查找/订阅中也不拦——迁移前
  // 这个 Dialog 没有任何 busy 门禁，所以这里不传 busy。
  return (
    <ReaderDialog
      title="添加订阅"
      ...
      onClose={onClose}
    >
      <p className="rss-dialog-lede">粘贴 RSS、Atom 地址或网站首页</p>
...
export function FolderDialog({ open, folder, onClose, onSave }: FolderDialogProps) {
  ...
  // 同样保留原规则：保存中也允许 backdrop / Escape / 关闭按钮关闭。
  return (
    <ReaderDialog
    ...
    >
      <form className="rss-dialog-body" onSubmit={submit}>
        <label className="rss-field">
          <span>名称</span>
          <input ref={nameRef} value={name} onChange={(event) => setName(event.target.value)} maxLength={128} />
        </label>
        {error && <div className="rss-dialog-error"><Icon name="alert" size={14} /> {error}</div>}
        <footer>
          <button type="button" className="rss-secondary-button" onClick={onClose}>取消</button>
