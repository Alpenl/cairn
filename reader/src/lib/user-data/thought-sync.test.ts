...
import { LEGACY_THOUGHT_OUTBOX_BLOCKED_REASON } from './thought-types'
...
function partialV6OutboxRecord(
  namespace: string,
  sequence: number,
  annotationId: string,
  wireIdentity: Record<string, unknown>,
...
async function replaceHistoryOutboxRecord(value: unknown): Promise<void> {
  const database = await openUserDataDatabase()
  if (!database) throw new Error('IndexedDB must be available in this test')
  await new Promise<void>((resolve, reject) => {
    const transaction = database.transaction(THOUGHT_HISTORY_OUTBOX_STORE, 'readwrite')
    transaction.objectStore(THOUGHT_HISTORY_OUTBOX_STORE).put(value)
...
function oldRemoteThought(id: string, sequence: number): Record<string, unknown> {
  return {
    id,
    host_kind: 'link',
    host_id: 'L1',
    ...
    annotation: {
      ...
      body: 'old wire thought',
      origin: 'user',
      source_content_revision: 7,
      created_at: 1,
      updated_at: 2,
      quote: { selected_text: 'old text', before: '', after: '' },
    },
  }
}
...
describe('durable thought sync loop', () => {
  ...
  it('quarantines partial v6 identities before allocating and dispatching a current v6 thought', async () => {
    const lease = identity('physical-A')
    const seeded = await runUserDataTransaction(
      lease,
      'seed partial v6 thoughts without replay-safe wire baselines',
      [THOUGHT_OUTBOX_STORE],
      'readwrite',
      (transaction, _identity, setResult) => {
        const rows = [
          partialV6OutboxRecord('physical-A', 50, 'partial-clock-zero', {
            opId: 'partial-clock-zero-op',
            deviceId: 'partial-device',
            contractVersion: 1,
            logicalClock: 0,
          }),
          partialV6OutboxRecord('physical-A', 51, 'partial-clock-negative', {
            opId: 'partial-clock-negative-op',
            deviceId: 'partial-device',
            contractVersion: 1,
            logicalClock: -1,
          }),
          partialV6OutboxRecord('physical-A', 52, 'partial-clock-fraction', {
            opId: 'partial-clock-fraction-op',
            deviceId: 'partial-device',
            contractVersion: 1,
            logicalClock: 1.5,
          }),
          partialV6OutboxRecord('physical-A', 53, 'partial-missing-device', {
            opId: 'partial-missing-device-op',
            contractVersion: 1,
            logicalClock: 1,
          }),
          partialV6OutboxRecord('physical-A', 54, 'partial-empty-device', {
            opId: 'partial-empty-device-op',
            deviceId: '',
            contractVersion: 1,
            logicalClock: 1,
          }),
          partialV6OutboxRecord('physical-A', 55, 'partial-whitespace-op', {
            opId: ' ',
            deviceId: 'partial-device',
            contractVersion: 1,
            logicalClock: 1,
          }),
          partialV6OutboxRecord('physical-A', 56, 'partial-missing-op', {
            deviceId: 'partial-device',
            contractVersion: 1,
            logicalClock: 1,
          }),
        ]
        for (const row of rows) transaction.objectStore(THOUGHT_OUTBOX_STORE).put(row)
        setResult(undefined)
      },
    )
    expect(seeded).toEqual({ ok: true, value: undefined })

    await expect(selectThoughtReadModel(lease)).resolves.toEqual({
      ok: true,
      value: expect.arrayContaining([
        expect.objectContaining({ id: 'partial-clock-zero', body: 'thought' }),
        expect.objectContaining({ id: 'partial-clock-negative', body: 'thought' }),
        expect.objectContaining({ id: 'partial-clock-fraction', body: 'thought' }),
        expect.objectContaining({ id: 'partial-missing-device', body: 'thought' }),
        expect.objectContaining({ id: 'partial-empty-device', body: 'thought' }),
        expect.objectContaining({ id: 'partial-whitespace-op', body: 'thought' }),
        expect.objectContaining({ id: 'partial-missing-op', body: 'thought' }),
      ]),
    })
    const partialBeforeCurrent = await readAll(THOUGHT_OUTBOX_STORE) as Array<Record<string, unknown>>
    expect(partialBeforeCurrent).toHaveLength(7)
    for (const row of partialBeforeCurrent) {
    ...
    expect(partialBeforeCurrent.find((row) => row.annotationId === 'partial-clock-zero'))
      .toMatchObject({ logicalClock: 0, deviceId: 'partial-device', opId: 'partial-clock-zero-op' })
    expect(partialBeforeCurrent.find((row) => row.annotationId === 'partial-empty-device'))
      .toMatchObject({ logicalClock: 1, deviceId: '', opId: 'partial-empty-device-op' })
    expect(partialBeforeCurrent.find((row) => row.annotationId === 'partial-whitespace-op'))
      .toMatchObject({ logicalClock: 1, deviceId: 'partial-device', opId: ' ' })
    expect(partialBeforeCurrent.find((row) => row.annotationId === 'partial-missing-device'))
      .not.toHaveProperty('deviceId')
    expect(partialBeforeCurrent.find((row) => row.annotationId === 'partial-missing-op'))
      .not.toHaveProperty('opId')

    await addLocalThought(lease, 'current-v6-op', 'current-v6-thought')
    expect(await readAll(THOUGHT_OUTBOX_STORE)).toEqual(expect.arrayContaining([
    ...
    const pushThoughtOps = vi.fn(async (request: Parameters<IdentityBoundReaderClient['pushThoughtOps']>[0]) => ({
      ok: true as const,
      data: request.ops.map((operation) => ({ op_id: operation.op_id, sequence: 20 })),
    }))
    ...
    await expect(syncThoughts(lease, client({ pushThoughtOps, syncThoughts: syncRemote })))
      .resolves.toMatchObject({
        ...
        pushed: 1,
        pending: 7,
        errorCode: LEGACY_THOUGHT_OUTBOX_BLOCKED_REASON,
      })
    expect(pushThoughtOps).toHaveBeenCalledTimes(1)
    expect(pushThoughtOps.mock.calls[0]?.[0].ops).toEqual([
      expect.objectContaining({ op_id: 'current-v6-op', contract_version: 1 }),
    ])
    const partialAfterAck = await readAll(THOUGHT_OUTBOX_STORE) as Array<Record<string, unknown>>
    expect(partialAfterAck).toHaveLength(7)
    expect(partialAfterAck).toEqual(expect.arrayContaining([
      expect.objectContaining({
        annotationId: 'partial-clock-zero',
        contractVersion: 1,
        logicalClock: 0,
        deviceId: 'partial-device',
        opId: 'partial-clock-zero-op',
      ...
      expect.objectContaining({
        annotationId: 'partial-whitespace-op',
        contractVersion: 1,
        logicalClock: 1,
        deviceId: 'partial-device',
        opId: ' ',
    ...
    resetUserDataDatabaseHandle()
    await expect(syncThoughts(identity('physical-A'), client({ pushThoughtOps, syncThoughts: syncRemote })))
      .resolves.toMatchObject({
        status: 'failed',
        pushed: 0,
        pending: 7,
    ...
    expect(pushThoughtOps).toHaveBeenCalledTimes(1)
    await expect(selectThoughtReadModel(identity('physical-A'))).resolves.toEqual({
      ok: true,
      value: expect.arrayContaining([
        expect.objectContaining({ id: 'partial-clock-zero', body: 'thought' }),
        expect.objectContaining({ id: 'partial-missing-op', body: 'thought' }),
  ...
  it('recovers a blocked compacted update tail without replaying or rewriting it', async () => {
    const lease = identity('physical-A')
    const legacyIdentity = {
      opId: 'legacy-compacted-update-op',
      deviceId: '',
      contractVersion: 0,
      logicalClock: 0,
    }
    const annotationId = 'legacy-compacted-update'
    const durableAnnotation = {
      ...annotationDraft(annotationId),
      note: 'materialized body before legacy tail',
      sourceContentRevision: 7,
      updatedAt: 1_000,
    }
    const seeded = await runUserDataTransaction(
      lease,
      'seed compacted legacy update tail',
      ...
      (transaction, _identity, setResult) => {
        transaction.objectStore(THOUGHT_OUTBOX_STORE).put({
          ...
          annotation: null,
          patch: {
            note: 'exact recovered compacted tail body',
            source: 'ai',
            updatedAt: 2_000,
          },
          createdAt: 1_000,
        ...
        transaction.objectStore(ANNOTATION_MATERIALIZED_STORE).put({
          key: ['physical-A', 'L1', 'saved-content:7', annotationId],
          namespace: 'physical-A',
          linkId: 'L1',
          target: TARGET,
          targetKey: 'saved-content:7',
          annotationId,
          sequence: 50,
          annotation: durableAnnotation,
          fallbackAnnotation: durableAnnotation,
        })
        setResult(undefined)
      },
    )
    ...
    if (!selected.ok) throw new Error('expected local compacted-tail projection')
    const recovered = selected.value.find((item) => item.id === annotationId)
    expect(recovered).toMatchObject({
      id: annotationId,
      body: 'exact recovered compacted tail body',
      source: 'ai',
      last_sequence: 0,
    })
    if (!recovered) throw new Error('missing local compacted-tail projection')
    const readModelItem: ThoughtReadModelItem = recovered
    expect(readModelItem).not.toHaveProperty('winner_key')
    // @ts-expect-error ThoughtReadModelItem deliberately has no wire winner key.
    expect(readModelItem.winner_key).toBeUndefined()

    const readLegacy = async (): Promise<Record<string, unknown>> => {
      const row = (await readAll(THOUGHT_OUTBOX_STORE)).find((candidate) =>
        (candidate as { annotationId?: string }).annotationId === annotationId)
      if (!row || typeof row !== 'object') throw new Error('missing compacted legacy tail')
      return row as Record<string, unknown>
    }
    const expectPreservedLegacyIdentity = async () => {
      ...
      }).toEqual(legacyIdentity)
      expect(row).toMatchObject({
        status: 'blocked',
        blockedReason: LEGACY_THOUGHT_OUTBOX_BLOCKED_REASON,
      })
      expect(row).not.toHaveProperty('nextAttemptAt')
    }
    ...
    const pushThoughtOps = vi.fn(async (
      ...
      ok: true as const,
      data: request.ops.map((operation) => ({ op_id: operation.op_id, sequence: 100 })),
    }))
    const syncRemote = vi.fn(async () => ({
      ok: true as const,
      data: { contract_version: 1 as const, items: [] },
    ...
    const stop = startThoughtSync(lease, thoughtClient)
    ...
    await expectPreservedLegacyIdentity()

    const online = Object.getOwnPropertyDescriptor(navigator, 'onLine')
    Object.defineProperty(navigator, 'onLine', { configurable: true, value: false })
    try {
      resetUserDataDatabaseHandle()
      await expect(syncThoughts(identity('physical-A', 2), thoughtClient)).resolves.toMatchObject({
        status: 'idle',
        pushed: 0,
        pending: 1,
      })
    ...
    await Promise.all([
      syncThoughts(identity('physical-A', 3), thoughtClient),
      syncThoughts(identity('physical-A', 4), thoughtClient),
    ])
    expect(pushThoughtOps).not.toHaveBeenCalled()
    await expectPreservedLegacyIdentity()

    const currentLease = identity('physical-A', 5)
    await addLocalThought(currentLease, 'current-after-compacted-tail', 'current-after-compacted-tail')
    expect(await readAll(THOUGHT_OUTBOX_STORE)).toEqual(expect.arrayContaining([
      expect.objectContaining({
        annotationId: 'current-after-compacted-tail',
        opId: 'current-after-compacted-tail',
        contractVersion: 1,
        logicalClock: 1,
      }),
    ]))

    await expect(syncThoughts(currentLease, thoughtClient)).resolves.toMatchObject({
      status: 'failed',
      pushed: 1,
      pending: 1,
      errorCode: LEGACY_THOUGHT_OUTBOX_BLOCKED_REASON,
    ...
    expect(pushThoughtOps.mock.calls[0]?.[0].ops).toEqual([
      expect.objectContaining({
        op_id: 'current-after-compacted-tail',
        contract_version: 1,
      }),
    ])
    const afterAck = await readAll(THOUGHT_OUTBOX_STORE) as Array<Record<string, unknown>>
    expect(afterAck).toHaveLength(1)
    expect(afterAck).toEqual([
      expect.objectContaining({ annotationId, status: 'blocked' }),
    ])
    await expectPreservedLegacyIdentity()
  })
  ...
  it('hydrates a legacy history candidate from its matching materialized winner key', async () => {
    const lease = identity('physical-A')
    await expect(syncThoughts(lease, client({
      syncThoughts: vi.fn(async () => ({
        ok: true as const,
        data: { contract_version: 1 as const, items: [remoteThought('history-thought', 11)] },
      })),
    }))).resolves.toMatchObject({ status: 'synced', pulled: 1 })
    await addHistoryReattach(lease, 'legacy-history-with-materialized-winner')
    const [stored] = await readAll(THOUGHT_HISTORY_OUTBOX_STORE)
    const legacy = structuredClone(stored) as { snapshot: Record<string, unknown> }
    delete legacy.snapshot.winnerKey
    await replaceHistoryOutboxRecord(legacy)

    const pushThoughtOps = vi.fn(async () => ({
      ok: false as const,
      error: { kind: 'network-unreachable' as const, message: 'retry later' },
    }))
    await expect(syncThoughts(lease, client({
      pushThoughtOps,
      syncThoughts: vi.fn(async () => ({
    ...
    }))).resolves.toMatchObject({ status: 'failed', pending: 1 })
    expect(pushThoughtOps).toHaveBeenCalledTimes(1)
    expect(await readAll(THOUGHT_HISTORY_OUTBOX_STORE)).toEqual([
      expect.objectContaining({
        opId: 'legacy-history-with-materialized-winner',
        snapshot: expect.objectContaining({
          winnerKey: {
            logicalClock: 11,
            deviceId: 'remote-device',
            opId: 'remote-history-thought-11',
          },
        }),
      }),
    ])
  ...
  it('quarantines a legacy history candidate without a materialized winner and keeps normal writes available', async () => {
    const lease = identity('physical-A')
    await addHistoryReattach(lease, 'legacy-history-without-materialized-winner')
    const [stored] = await readAll(THOUGHT_HISTORY_OUTBOX_STORE)
    const legacy = structuredClone(stored) as { snapshot: Record<string, unknown> }
    delete legacy.snapshot.winnerKey
    await replaceHistoryOutboxRecord(legacy)
    ...
    }))).resolves.toMatchObject({ status: 'idle', pushed: 0, pending: 0 })
    expect(pushThoughtOps).not.toHaveBeenCalled()
    expect(await readAll(THOUGHT_HISTORY_OUTBOX_STORE)).toEqual([
      expect.objectContaining({
        opId: 'legacy-history-without-materialized-winner',
        status: 'blocked',
        blockedReason: 'missing-server-winner-key',
        snapshot: expect.not.objectContaining({ winnerKey: expect.anything() }),
      }),
    ])

    await addLocalThought(lease, 'after-legacy-history-quarantine', 'after-legacy-history-quarantine')
    expect(await readAll(THOUGHT_OUTBOX_STORE)).toEqual([
      expect.objectContaining({ opId: 'after-legacy-history-quarantine', logicalClock: 13 }),
    ])
  })
  ...
  it('dual-reads an old annotation response and derives its target from source identity', async () => {
    const lease = identity('physical-A')
    const syncRemote = vi.fn()
      .mockResolvedValueOnce({
        ok: true as const,
        data: { items: [oldRemoteThought('old-a1', 8)] },
      })
    await expect(syncThoughts(lease, client({
      pushThoughtOps: vi.fn(),
      syncThoughts: syncRemote,
    ...
    await expect(listRemoteThoughts(lease, 'L1', TARGET)).resolves.toMatchObject({
      ...
      value: [expect.objectContaining({
        ...
        body: 'old wire thought',
        serverSequence: 8,
        quote: expect.objectContaining({ exact: 'old text', start: 0, end: 4 }),
      })],
    })
  })
...
describe('observable Thought sync controller', () => {
  ...
  it('accepts one content-free repair wake for the current namespace and ignores invalid, duplicate, and disposed messages', async () => {
    class FakeBroadcastChannel {
      static readonly instances = new Set<FakeBroadcastChannel>()
      readonly listeners = new Set<(event: MessageEvent<unknown>) => void>()
    ...
    const syncRemote = vi.fn<IdentityBoundReaderClient['syncThoughts']>(() =>
      new Promise((resolve) => {
        releasePull = () => resolve({
          ok: true,
          data: { contract_version: 1, items: [] },
        })
      }),
    )
    const lease = identity('repair-current')
    const controller = getThoughtSyncController(lease, client({
      pushThoughtOps: vi.fn(),
      syncThoughts: syncRemote,
    }))
    const stop = controller.start()
    const sender = new FakeBroadcastChannel('webtag-reader-thought-sync-v1')
    try {
      Object.defineProperty(window.navigator, 'onLine', { configurable: true, value: true })
      sender.postMessage({ kind: 'thought-repair-quarantine', namespace: 'repair-current', reasons: [] })
      sender.postMessage({
        kind: 'thought-repair-quarantine',
        namespace: 'repair-other',
        reasons: [{ reason: 'invalid_quote', count: 1 }],
      })
      sender.postMessage({
        kind: 'thought-repair-quarantine',
        namespace: 'repair-current',
        reasons: [{ reason: 'invalid_quote', count: 0 }],
      })
      ...
      expect(valid).not.toHaveProperty('quote')
      expect(valid).not.toHaveProperty('target')
      expect(valid).not.toHaveProperty('payload')
      releasePull()
      await vi.waitFor(() => expect(controller.getSnapshot()).toMatchObject({ phase: 'synced' }))
  ...
  it('keeps a lock-losing legacy snapshot entirely read-only', async () => {
    class FakeBroadcastChannel {
      static readonly messages: unknown[] = []

      constructor(_name: string) {}

      addEventListener(_type: 'message', _listener: EventListener): void {}

      removeEventListener(_type: 'message', _listener: EventListener): void {}
    ...
    try {
      const lease = identity('lock-loser-read-only')
      await addLocalThought(lease, 'legacy-lock-op', 'legacy-lock-thought')
      const database = await openUserDataDatabase()
      if (!database) throw new Error('IndexedDB must be available in this test')
      await new Promise<void>((resolve, reject) => {
        const transaction = database.transaction(THOUGHT_OUTBOX_STORE, 'readwrite')
        const store = transaction.objectStore(THOUGHT_OUTBOX_STORE)
        const row = store.get([lease.context.physicalNamespace, 1]) as IDBRequest<ThoughtOutboxRecord | undefined>
        row.onsuccess = () => {
          if (!row.result) {
            transaction.abort()
            return
          ...
          store.put({
            ...
            lastError: 'legacy transport detail that normalization would remove',
          } satisfies ThoughtOutboxRecord)
        }
        row.onerror = () => transaction.abort()
        transaction.oncomplete = () => resolve()
        transaction.onerror = () => reject(transaction.error)
        transaction.onabort = () => reject(transaction.error)
      })
      ...
      const before = {
        state: structuredClone(await readAll(THOUGHT_SYNC_STATE_STORE)),
        outbox: structuredClone(await readAll(THOUGHT_OUTBOX_STORE)),
      }
      expect(before.outbox).toEqual([expect.objectContaining({
        contractVersion: 0,
        logicalClock: 0,
        lastError: 'legacy transport detail that normalization would remove',
      })])
      ...
      await expect(controller.sync()).resolves.toMatchObject({ status: 'idle', pending: 1 })
      ...
      expect(FakeBroadcastChannel.messages).toEqual([])
      expect({
        state: await readAll(THOUGHT_SYNC_STATE_STORE),
        outbox: await readAll(THOUGHT_OUTBOX_STORE),
      }).toEqual(before)
    } finally {
      stop?.()
      vi.unstubAllGlobals()
