...
func (s *pgSitePayloadCleanupStore) ListExpired(ctx context.Context, limit int) ([]sitePayloadCandidate, error) {
	...
	candidates := make([]sitePayloadCandidate, 0, alloc.Hint(limit))
	for rows.Next() {
...
func NewSitePayloadCleaner(opts SitePayloadCleanerOptions) (*SitePayloadCleaner, error) {
	...
	return newSitePayloadCleanerWithMetrics(&pgSitePayloadCleanupStore{db: opts.Pool}, opts.Interval, opts.Batch, opts.Logger, opts.Metrics), nil
}
...
func newSitePayloadCleaner(store sitePayloadCleanupStore, interval time.Duration, batch int, logger *slog.Logger) *SitePayloadCleaner {
	return newSitePayloadCleanerWithMetrics(store, interval, batch, logger, nil)
}

func newSitePayloadCleanerWithMetrics(store sitePayloadCleanupStore, interval time.Duration, batch int, logger *slog.Logger, metrics *observability.Metrics) *SitePayloadCleaner {
	if interval <= 0 {
	...
	return &SitePayloadCleaner{
		...
		logger: logger, metrics: metrics, lifecycle: newBackgroundLoop("site_payload_cleaner", logger),
	}
}
...
func (c *SitePayloadCleaner) RunOnce(ctx context.Context) (int, error) {
	...
	for _, candidate := range candidates {
		...
		if didPurge {
			purged++
			c.recordPurge("success")
		} else {
			c.recordPurge("skipped")
		}
	}
	return purged, nil
}

func (c *SitePayloadCleaner) recordPurge(result string) {
	if c != nil && c.metrics != nil && c.metrics.SitePayloadPurgeTotal != nil {
		c.metrics.SitePayloadPurgeTotal.WithLabelValues("deadline", result).Inc()
	}
}
