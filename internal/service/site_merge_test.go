...
func (f siteMergeReaderFake) ListSites(context.Context, repository.SiteListFilter) ([]repository.SiteListItem, int, error) {
	return nil, 0, nil
}
...
func TestSiteMergePreviewExposesUserConflictsAndDuplicates(t *testing.T) {
	targetID, sourceID := uuid.New(), uuid.New()
	user := model.FieldSourceUser
	target := &repository.SiteDetail{SiteListItem: repository.SiteListItem{Site: model.Site{ID: targetID, Name: "Target", NameSource: user, Intro: "same auto intro", IntroSource: model.FieldSourceAuto, UserNote: "target note", Revision: 4}}, Entries: []model.SiteEntry{{ID: uuid.New(), SiteID: targetID, LinkID: uuid.New(), EntryName: "Home", NormalizedURL: "https://example.com/"}}, Tags: []model.SiteTag{{Tag: "Go", NormalizedTag: "go", Source: user}}, Identities: []model.SiteIdentity{{IdentityKey: "v1:host:example.com"}}}
	source := &repository.SiteDetail{SiteListItem: repository.SiteListItem{Site: model.Site{ID: sourceID, Name: "Source", NameSource: user, Intro: "different auto intro", IntroSource: model.FieldSourceAuto, UserNote: "source note", Revision: 7}}, Entries: []model.SiteEntry{{ID: uuid.New(), SiteID: sourceID, LinkID: uuid.New(), EntryName: "Duplicate", NormalizedURL: "https://example.com/"}, {ID: uuid.New(), SiteID: sourceID, LinkID: uuid.New(), EntryName: "Docs", NormalizedURL: "https://example.com/docs"}}, Tags: []model.SiteTag{{Tag: "Tools", NormalizedTag: "tools", Source: user}}, Identities: []model.SiteIdentity{{IdentityKey: "v1:host:docs.example.com"}}}
	...
	if len(got.UserTags) != 2 || got.UserTags[0] != "Go" || got.UserTags[1] != "Tools" {
		t.Fatalf("tags=%v", got.UserTags)
	}
	if len(got.IdentityKeys) != 2 || got.IdentityKeys[0] != "v1:host:docs.example.com" {
		t.Fatalf("identities=%v", got.IdentityKeys)
	}
	if !got.RequiresResolution || len(got.FieldConflicts) != 2 || got.FieldConflicts[0].Field != "name" || got.FieldConflicts[1].Field != "user_note" {
...
func TestSiteMergeExecuteRequiresEveryPreviewConflictToBeResolved(t *testing.T) {
	targetID, sourceID := uuid.New(), uuid.New()
	user := model.FieldSourceUser
	target := &repository.SiteDetail{SiteListItem: repository.SiteListItem{Site: model.Site{ID: targetID, Name: "Target", NameSource: user, Revision: 4}}}
	source := &repository.SiteDetail{SiteListItem: repository.SiteListItem{Site: model.Site{ID: sourceID, Name: "Source", NameSource: user, Revision: 7}}}
...
var _ repository.SiteReader = siteMergeReaderFake{}
var _ repository.SiteMergeWriter = (*siteMergeExecuteFake)(nil)
