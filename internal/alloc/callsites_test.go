...
var hintCallSites = map[string]string{
	...
	"internal/repository/reader_vnext.go:6486":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（semanticRelatedTags）",
	"internal/repository/reader_vnext.go:6535":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（cooccurrenceRelatedTags）",
	"internal/repository/reader_vnext.go:6692":               "本函数内联 clamp `query.Limit > 1000 → 100`（ListActivity 的 limit+1 页）",
	"internal/repository/reader_vnext.go:6846":               "本函数内联 clamp `limit > 100 → 50`（ListContentHistory）",
