...
var hintCallSites = map[string]string{
	...
	"internal/repository/reader_host_lifecycle.go:488":       "同一分配点调用 normalizeReaderTrashLimit，夹到 readerTrashMaxLimit=200（非法值回退 50）",
	"internal/repository/reader_vnext.go:1601":               "本函数内联 clamp `limit > 200 → 50`（ListThoughts）",
	"internal/repository/reader_vnext.go:1686":               "本函数内联 clamp `limit > 100 → 20`（SearchThoughts）",
	"internal/repository/reader_vnext.go:1907":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtsSince）",
	"internal/repository/reader_vnext.go:1954":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtConflicts）",
	"internal/repository/reader_vnext.go:2003":               "本函数内联 clamp `limit > 100 → 30`（ListThoughtHistory）",
	"internal/repository/reader_vnext.go:2546":               "本函数内联 clamp `limit > 100 → 30`（ListNotes）",
	"internal/repository/reader_vnext.go:2589":               "本函数内联 clamp `limit > 100 → 20`（SearchPublishedNotes）",
	"internal/repository/reader_vnext.go:3317":               "本函数内联 clamp `limit > 100 → 50`（ListNoteHistory）",
	"internal/repository/reader_vnext.go:3479":               "本函数内联 clamp `limit > 100 → 30`（ListInbox）",
	"internal/repository/reader_vnext.go:3541":               "本函数内联 clamp `limit > 500 → 100`（ClaimExpiredInbox，由 worker 的 batch size 驱动）",
	"internal/repository/reader_vnext.go:5134":               "本函数内联 clamp `limit > 10 → 3`（ListContinueReading；当前无调用方，接线前先看这条）",
	"internal/repository/reader_vnext.go:6606":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（semanticRelatedTags）",
	"internal/repository/reader_vnext.go:6655":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（cooccurrenceRelatedTags）",
	"internal/repository/reader_vnext.go:6812":               "本函数内联 clamp `query.Limit > 1000 → 100`（ListActivity 的 limit+1 页）",
	"internal/repository/reader_vnext.go:6966":               "本函数内联 clamp `limit > 100 → 50`（ListContentHistory）",
