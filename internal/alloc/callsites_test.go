...
var hintCallSites = map[string]string{
	...
	"internal/repository/reader_vnext.go:1611":               "本函数内联 clamp `limit > 200 → 50`（ListThoughts）",
	"internal/repository/reader_vnext.go:1696":               "本函数内联 clamp `limit > 100 → 20`（SearchThoughts）",
	"internal/repository/reader_vnext.go:1917":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtsSince）",
	"internal/repository/reader_vnext.go:1964":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtConflicts）",
	"internal/repository/reader_vnext.go:2013":               "本函数内联 clamp `limit > 100 → 30`（ListThoughtHistory）",
	"internal/repository/reader_vnext.go:2556":               "本函数内联 clamp `limit > 100 → 30`（ListNotes）",
	"internal/repository/reader_vnext.go:2599":               "本函数内联 clamp `limit > 100 → 20`（SearchPublishedNotes）",
	"internal/repository/reader_vnext.go:3327":               "本函数内联 clamp `limit > 100 → 50`（ListNoteHistory）",
	"internal/repository/reader_vnext.go:3491":               "本函数内联 clamp `limit > 100 → 30`（ListInbox）",
	"internal/repository/reader_vnext.go:3553":               "本函数内联 clamp `limit > 500 → 100`（ClaimExpiredInbox，由 worker 的 batch size 驱动）",
	"internal/repository/reader_vnext.go:5169":               "本函数内联 clamp `limit > 10 → 3`（ListContinueReading；当前无调用方，接线前先看这条）",
	"internal/repository/reader_vnext.go:6641":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（semanticRelatedTags）",
	"internal/repository/reader_vnext.go:6690":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（cooccurrenceRelatedTags）",
	"internal/repository/reader_vnext.go:6847":               "本函数内联 clamp `query.Limit > 1000 → 100`（ListActivity 的 limit+1 页）",
	"internal/repository/reader_vnext.go:7001":               "本函数内联 clamp `limit > 100 → 50`（ListContentHistory）",
