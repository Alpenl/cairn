...
var hintCallSites = map[string]string{
	...
	"internal/repository/reader_vnext.go:1834":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtsSince）",
	"internal/repository/reader_vnext.go:1881":               "本函数内联 clamp `limit > 200 → 100`（ListThoughtConflicts）",
	"internal/repository/reader_vnext.go:1930":               "本函数内联 clamp `limit > 100 → 30`（ListThoughtHistory）",
	"internal/repository/reader_vnext.go:2459":               "本函数内联 clamp `limit > 100 → 30`（ListNotes）",
	"internal/repository/reader_vnext.go:2502":               "本函数内联 clamp `limit > 100 → 20`（SearchPublishedNotes）",
	"internal/repository/reader_vnext.go:3225":               "本函数内联 clamp `limit > 100 → 50`（ListNoteHistory）",
	"internal/repository/reader_vnext.go:3380":               "本函数内联 clamp `limit > 100 → 30`（ListInbox）",
	"internal/repository/reader_vnext.go:3442":               "本函数内联 clamp `limit > 500 → 100`（ClaimExpiredInbox，由 worker 的 batch size 驱动）",
	"internal/repository/reader_vnext.go:5038":               "本函数内联 clamp `limit > 10 → 3`（ListContinueReading；当前无调用方，接线前先看这条）",
	"internal/repository/reader_vnext.go:6470":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（semanticRelatedTags）",
	"internal/repository/reader_vnext.go:6519":               "无本函数 clamp：limit 由唯一调用方 RelatedTags 夹到 `> 50 → 12`（cooccurrenceRelatedTags）",
	"internal/repository/reader_vnext.go:6676":               "本函数内联 clamp `query.Limit > 1000 → 100`（ListActivity 的 limit+1 页）",
	"internal/repository/reader_vnext.go:6830":               "本函数内联 clamp `limit > 100 → 50`（ListContentHistory）",
