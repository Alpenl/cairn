 ...
 *   - getSessionIdentity GET /api/session
 *   - getReaderPendingCount GET /api/home
 ...
 *   - testConnection  轻量鉴权探活（复用 GET /api/capabilities）
 *
 ...
 *   - timeout             请求超时
 *   - rate-limited        429 限流 / 冷却中（全局 rate_limit_exceeded 等）。
 *                         如后端给出 Retry-After 头，
 ...
 *   - Link / LinkContent / SubmitResponse 在 wire 边界按 generated contract 完整
 *     校验；不补缺失字段、不过滤残缺数组项、不把非法 enum 强转为合法类型。
 *
 * 消费者：测试连接 UI、采集编排、RSS 订阅、Reader badge compatibility。
 */
