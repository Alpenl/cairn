...
class SettingsQueueGroupingTest {
    ...
    fun everyQueueStateIsCoveredByTheFixtureAndLandsInItsDeclaredSection() {
        ...
        // Six states, six declarations: a seventh state added to the enum without a fixture entry
    ...
    fun theTotalCountsDurableRowsNotRenderedSections() {
        // Five sections, one row each: a total derived from `groups.size` would read 5 here too, so
        ...
        val rows = listOf(
            ...
            row("d", QueueState.BLOCKED_IDENTITY),
            row("e", QueueState.FAILED_PERMANENT),
            row("f", QueueState.EXPIRED),
        )

        val projection = SettingsQueuePresenter.project(rows)

        assertEquals(6, projection.total)
        assertEquals(5, projection.groups.size)
        assertEquals(6, projection.groups.sumOf { it.count })
    }
