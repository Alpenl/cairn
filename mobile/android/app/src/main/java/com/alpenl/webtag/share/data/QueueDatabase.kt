...
import com.alpenl.webtag.share.contract.INITIAL_ACTIVATION_REVISION
...
sealed interface QueueDueClaimOutcome {
    data class Claimed(val entry: QueueEntity, val owner: String) : QueueDueClaimOutcome
    data object NoWork : QueueDueClaimOutcome
    data object IdentityChanged : QueueDueClaimOutcome
}
...
interface QueueDao {
    ...
    @Query(
        "UPDATE queue_entries SET state = 'pending_submit', firstFailedAt = :firstFailedAt, " +
            ...
            "WHERE id = :id " +
            "AND state IN ('retry_wait', 'blocked_auth', 'failed_permanent', 'expired', " +
            "'RETRY_WAIT', 'BLOCKED_AUTH', 'FAILED_PERMANENT', 'EXPIRED') " +
            "AND (leaseOwner IS NULL OR leaseExpiresAt <= :now)",
    )
    ...
    companion object {
        ...
        val MIGRATION_1_2 = object : Migration(1, 2) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE todo_outbox ADD COLUMN activationRevision INTEGER NOT NULL DEFAULT 0",
                )
                database.execSQL(
                    "ALTER TABLE todo_outbox ADD COLUMN envelopeVersion INTEGER NOT NULL DEFAULT 1",
                )
                database.execSQL("ALTER TABLE todo_outbox ADD COLUMN quarantinedLegacyState TEXT")
                database.execSQL(
                    "ALTER TABLE todo_sync_state ADD COLUMN activationRevision INTEGER NOT NULL DEFAULT 0",
                )
                database.execSQL(
                    "ALTER TABLE todo_sync_state ADD COLUMN gateState TEXT NOT NULL DEFAULT 'ready'",
                )
                database.execSQL(
                    "ALTER TABLE todo_sync_state ADD COLUMN attemptCount INTEGER NOT NULL DEFAULT 0",
                )
                database.execSQL("ALTER TABLE todo_sync_state ADD COLUMN nextAttemptAt INTEGER")
                database.execSQL("ALTER TABLE todo_sync_state ADD COLUMN lastErrorKind TEXT")
                database.execSQL(
                    "DROP INDEX IF EXISTS index_todo_outbox_apiOrigin_clientDataNamespace_state_nextAttemptAt",
                )
                database.execSQL(
                    "CREATE INDEX IF NOT EXISTS " +
                        "index_todo_outbox_apiOrigin_clientDataNamespace_activationRevision_state_nextAttemptAt " +
                        "ON todo_outbox (apiOrigin, clientDataNamespace, activationRevision, state, nextAttemptAt)",
                )
                migrateLegacyTodoOutbox(database)
            }
        }
...
class QueueRepository(
    ...
    fun claimDue(
    ...
    ): QueueDueClaimOutcome = database.runInTransaction(java.util.concurrent.Callable {
        if (!activeSessionMatches(activation)) return@Callable QueueDueClaimOutcome.IdentityChanged
        val identity = activation.identity
        val entry = dao.findDue(identity.origin, identity.clientDataNamespace, now)
            ?: return@Callable QueueDueClaimOutcome.NoWork
        val owner = newUuid()
        ...
        if (dao.claim(
            ...
            return@Callable if (activeSessionMatches(activation)) QueueDueClaimOutcome.NoWork
            else QueueDueClaimOutcome.IdentityChanged
        }
        QueueDueClaimOutcome.Claimed(entry.copy(leaseOwner = owner, leaseExpiresAt = expiresAt), owner)
    })
    ...
    fun claimForEntry(
        ...
        now: Long = System.currentTimeMillis(),
        leaseMillis: Long = 30_000,
    ): SubmitClaimOutcome = database.runInTransaction(java.util.concurrent.Callable {
        if (!activeSessionMatches(activation)) return@Callable SubmitClaimOutcome.IDENTITY_CHANGED
        val identity = activation.identity
        val expiresAt = runCatching { Math.addExact(now, leaseMillis) }.getOrDefault(Long.MAX_VALUE)
        val updated = dao.claim(
            entry.id,
            entry.apiOrigin,
            entry.clientDataNamespace,
            owner,
        ...
        if (updated == 1) SubmitClaimOutcome.APPLIED
        else if (activeSessionMatches(activation)) SubmitClaimOutcome.STALE_CLAIM
        else SubmitClaimOutcome.IDENTITY_CHANGED
    })
    ...
    fun activateSessionIfLatest(
        ...
        return database.runInTransaction(java.util.concurrent.Callable {
            ...
            val activation = ActiveSessionSnapshot(identity, generation)
            TodoRepository.prepareActivation(database.todoDao(), cipher, activation)
            dao.upsertActiveSession(activeSessionEntity(identity, generation))
            ActivationCommitOutcome.APPLIED
        })
    }

    /** Test/setup compatibility; production activation must use [activateSessionIfLatest]. */
    fun activateSession(identity: SessionIdentity, revision: Long = INITIAL_ACTIVATION_REVISION) {
        validateSessionIdentity(identity)
        ...
        database.runInTransaction {
            ...
            TodoRepository.prepareActivation(
                database.todoDao(),
                cipher,
                ActiveSessionSnapshot(identity, revision),
            )
    ...
    fun activeSessionSnapshot(): ActiveSessionSnapshot? {
        ...
        check(session.representationContract == REPRESENTATION_CONTRACT) {
            "active session contract is unsupported"
        }
    ...
    private fun validateSessionIdentity(identity: com.alpenl.webtag.share.contract.SessionIdentity) {
        ...
        check(identity.isSupported()) { "unsupported session contract" }
    }
