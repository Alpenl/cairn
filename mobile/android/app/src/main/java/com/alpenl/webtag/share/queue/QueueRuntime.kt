...
class ConnectionCoordinator(
    ...
    fun saveAndTest(rawOrigin: String, installationToken: String): ConnectionResult {
        ...
        val response = runCatching { api.validateSession(normalized, installationToken) }
        ...
        return when (response) {
            ...
            is ApiResult.Success -> activate(generation, normalized, installationToken, response.value)
        }
    }
    ...
    private fun activate(
        ...
        if (!identity.isSupported()) return latestFailure(generation, ErrorKind.INVALID_CLIENT_RESPONSE)
...
class RefreshCoordinator(
    ...
    fun refresh(selected: RecentResult): RefreshResult {
        ...
        val attempt = when (val session = api.validateSession(config.origin, config.installationToken)) {
            is ApiResult.Failure -> RefreshAttempt.Failure(session.failure)
            is ApiResult.Success -> {
                val verified = session.value
                if (!verified.isSupported() || verified != activation.identity) {
                    RefreshAttempt.Failure(ClassifiedFailure(ErrorKind.IDENTITY_MISMATCH))
                } else {
                    when (val response = api.refresh(verified, config.installationToken, selected.linkId)) {
...
class ShareSubmissionCoordinator(
    ...
    fun submit(
        ...
        if (identity == null || !identity.isSupported()) return SubmissionOutcome.ConfigurationRequired
        val activation = runCatching { repository.activeSessionSnapshot() }.getOrNull()
        ...
        val installationToken = config.installationToken
        ...
        when (runCatching { repository.claimForEntry(entry, owner, activation, now) }
            .getOrDefault(SubmitClaimOutcome.STALE_CLAIM)) {
            SubmitClaimOutcome.APPLIED -> Unit
            SubmitClaimOutcome.IDENTITY_CHANGED -> return SubmissionOutcome.IdentityChanged
            SubmitClaimOutcome.STALE_CLAIM -> {
                runCatching { scheduler.schedule(now) }
                return SubmissionOutcome.Queued(
        ...
        val result = runCatching { api.submit(identity, installationToken, url, entry.idempotencyKey) }
    ...
    fun drainOne(now: Long = System.currentTimeMillis()): Boolean {
        ...
        if (!identity.isSupported() || config.sessionIdentity() != identity ||
            config.activationRevision != activation.activationRevision
        ) {
            return false
        }
        val claimed = runCatching { repository.claimDue(activation, now) }
            .getOrDefault(QueueDueClaimOutcome.NoWork)
        val (entry, owner) = when (claimed) {
            is QueueDueClaimOutcome.Claimed -> claimed.entry to claimed.owner
            QueueDueClaimOutcome.NoWork, QueueDueClaimOutcome.IdentityChanged -> return false
        }
        ...
        val result = runCatching { api.submit(identity, config.installationToken, url, entry.idempotencyKey) }
            .getOrElse { ApiResult.Failure(ClassifiedFailure(ErrorKind.LOCAL_DATA_UNREADABLE), null) }
...
class MobileRuntime private constructor(context: Context) {
    ...
    val todoScheduler = TodoSyncScheduler(appContext, todoRepository, repository::activeSessionSnapshot)
