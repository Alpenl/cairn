...
interface WebTagApi {
    fun validateSession(rawOrigin: String, installationToken: String): ApiResult<SessionIdentity>
...
interface WebTagCompanionApi {
    fun capabilities(identity: SessionIdentity, installationToken: String): ApiResult<TodoCapabilities>

    fun listTodos(identity: SessionIdentity, installationToken: String): ApiResult<List<TodoItem>>
    ...
    fun home(identity: SessionIdentity, installationToken: String): ApiResult<HomeSnapshot>
}

class WebTagApiClient(
    private val httpClient: OkHttpClient = defaultHttpClient(),
) : WebTagApi, WebTagCompanionApi {
    override fun validateSession(rawOrigin: String, installationToken: String): ApiResult<SessionIdentity> {
        ...
        return execute(origin, installationToken, "GET", "/api/session", null, acceptedStatusCodes = setOf(200)) { body, namespace ->
            ...
            require(isNamespace(bodyNamespace) && representation == com.alpenl.webtag.share.contract.REPRESENTATION_CONTRACT) {
                "unsupported session contract"
            }
            SessionIdentity(origin, bodyNamespace, representation)
        }
    }
    ...
    override fun submit(
        ...
        if (!identity.isSupported() || !isNamespace(identity.clientDataNamespace) || installationToken.isBlank() || idempotencyKey.isBlank() ||
    ...
    override fun refresh(
        ...
        if (!identity.isSupported() || !isNamespace(identity.clientDataNamespace) || installationToken.isBlank() ||
            runCatching { OriginNormalizer.normalize(identity.origin) }.getOrNull() != identity.origin ||
            !isUuid(linkId)
        ) {
            return ApiResult.Failure(
    ...
    override fun capabilities(identity: SessionIdentity, installationToken: String): ApiResult<TodoCapabilities> {
        if (!validIdentityRequest(identity, installationToken)) return invalidRequest()
        return execute(
            identity.origin,
            installationToken,
            "GET",
    ...
    override fun createTodo(
        ...
        if (!validIdentityRequest(identity, installationToken) || request.text.isBlank() ||
    ...
    override fun patchTodo(
        ...
        if (!validIdentityRequest(identity, installationToken) || !isUuid(todoId) || idempotencyKey.isBlank() ||
    ...
    private fun validIdentityRequest(identity: SessionIdentity, installationToken: String): Boolean =
        identity.isSupported() && isNamespace(identity.clientDataNamespace) && installationToken.isNotBlank() &&
            runCatching { OriginNormalizer.normalize(identity.origin) }.getOrNull() == identity.origin
