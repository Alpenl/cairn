...
internal class RuntimeCompanionSnapshotSource(
    ...
    suspend fun read(): Pair<Long, CompanionSnapshot> = withContext(Dispatchers.IO) {
        ...
        val activation = runCatching { runtime.repository.activeSessionSnapshot() }.getOrNull()
            ?.takeIf { snapshot ->
                current != null &&
                    current.activationRevision == snapshot.activationRevision &&
                    current.sessionIdentity() == snapshot.identity
            }
        val identity = activation?.let {
            QueueIdentity(it.identity.origin, it.identity.clientDataNamespace)
        }
        ...
        val todos = activation?.let {
            when (val result = runCatching { runtime.todoRepository.snapshot(it) }.getOrNull()) {
                is TodoSnapshotOutcome.Applied -> result.snapshot
                else -> TodoLocalSnapshot.EMPTY
            }
        } ?: TodoLocalSnapshot.EMPTY
        val revision = activation?.activationRevision ?: SettingsSnapshot.NO_ACTIVE_SESSION
        revision to CompanionSnapshot(
            settings = SettingsProjection(
                activationRevision = revision,
                activeNamespace = activation?.identity?.clientDataNamespace,
