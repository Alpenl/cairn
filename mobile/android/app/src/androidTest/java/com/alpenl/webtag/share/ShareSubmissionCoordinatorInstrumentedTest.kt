...
class ShareSubmissionCoordinatorInstrumentedTest {
    ...
    fun setUp() {
        ...
        identity = SessionIdentity(origin, namespace, REPRESENTATION_CONTRACT)
        credentials.save(CredentialConfig(origin, "test-token", namespace))
    ...
    fun authFailureIsRetainedAsBlockedAuthWithNoRetrySchedule() {
        server.enqueue(
            MockResponse()
                .setResponseCode(401)
                .setBody("{\"error\":{\"error_code\":\"unauthorized\"}}"),
        )
    ...
    fun activeLeaseIsReusedWithoutCreatingOrSendingAnotherRow() {
        ...
        val activation = repository.activeSessionSnapshot()!!
        assertEquals(1, database.queueDao().claim(
            id = entry.id,
            apiOrigin = entry.apiOrigin,
            namespace = entry.clientDataNamespace,
            owner = "active-owner",
            expiresAt = now + 30_000,
            now = now,
            activeOrigin = activation.identity.origin,
