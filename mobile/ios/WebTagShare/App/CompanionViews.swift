...
final class CompanionAppModel: ObservableObject {
    ...
    private var activeActivation: ActivationIdentity?
    ...
    private var todoStateObserver: NSObjectProtocol?
    ...
    init(
        ...
        reload()
        todoStateObserver = NotificationCenter.default.addObserver(
    ...
    deinit {
        if let todoStateObserver { NotificationCenter.default.removeObserver(todoStateObserver) }
    }

    func reload() {
        do {
            guard let activation = try queueRepository?.activeSessionSnapshot(),
                  let stored = try credentials.loadConfig(),
                  stored.activation == activation else {
            ...
            activeIdentity = activation.queueIdentity
            todos = try todoRepository?.snapshot(activation: activation) ?? .empty
        } catch {
            activeActivation = nil
            activeIdentity = nil
            todos = .empty
    ...
    func create(text: String, dueAt: Date?) {
        stage { activation, repository in
    ...
    func delete(_ item: CompanionTodoItem) {
        guard item.isStandalone else { return }
        stage { activation, repository in
            try repository.stageDelete(activation: activation, todoID: item.id)
        }
    }
    ...
    private func stage(_ body: (ActivationIdentity, CompanionTodoRepository) throws -> Void) {
        guard let activation = activeActivation, let todoRepository else {
            syncStatus = "请先配置服务器连接"
            return
        }
        do {
            try body(activation, todoRepository)
            syncStatus = "已保存，等待同步"
            reload()
            synchronize()
        } catch {
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                if model.activeIdentity == nil {
                    Section {
                        CompanionStatusView(title: "尚未连接", detail: "请先设置服务器地址和安装令牌。")
                        Button("设置连接", action: openSettings)
                    }
                        ...
                        Section("\(group.group.title) · \(group.count) 条") {
                            ForEach(group.rows, id: \.id) { entry in
                                QueueEntryRow(
                                    ...
                                    canMigrate: settings.canMigrate(entry),
