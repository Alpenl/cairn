...
private let supportedRepresentationContract = "v3"
...
struct ActivationIdentity: Equatable {
    let identity: SessionIdentity
    let revision: Int64

    var queueIdentity: QueueIdentity {
        QueueIdentity(origin: identity.origin, namespace: identity.namespace)
    }
}

typealias ActiveSessionSnapshot = ActivationIdentity
...
struct SessionIdentity: Equatable {
    ...
    var isSupported: Bool { representationContract == supportedRepresentationContract }
}
...
struct CredentialConfig: Equatable {
    let activation: ActivationIdentity
    let installationToken: String

    var identity: SessionIdentity { activation.identity }
    var activationRevision: Int64 { activation.revision }

    init(activation: ActivationIdentity, installationToken: String) {
        self.activation = activation
        self.installationToken = installationToken
    }

    /// Kept for source compatibility in test fixtures. Production Keychain
    /// decoding never supplies a default revision.
    init(identity: SessionIdentity, installationToken: String, activationRevision: Int64 = 1) {
        activation = ActivationIdentity(identity: identity, revision: activationRevision)
        self.installationToken = installationToken
    }
}
...
struct RecentMigrationCandidate: Equatable, Identifiable {
    let identity: QueueIdentity
    let revision: Int64
    let createdAt: Date
...
private func validateSessionIdentity(_ identity: SessionIdentity) throws {
    ...
    guard identity.representationContract == supportedRepresentationContract else {
...
enum QueueFailurePolicy {
    static func state(for category: ErrorCategory, firstFailedAt: Date?, now: Date) -> QueueState {
        ...
        case .forbidden: return .blockedAuth
...
final class KeychainCredentialStore: CredentialConfigLoading {
    private let account = "installation-credential-v1"
    private let retiredAccounts = ["credential-v2", "credential-v1"]

    func save(config: CredentialConfig) throws {
        try validateSessionIdentity(config.identity)
        guard config.activationRevision > 0,
              !config.installationToken.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw CoreError.keychain
        }
        let object: [String: Any] = [
            "installation_token": config.installationToken,
            ...
            "activation_revision": config.activationRevision,
        ]
        ...
        retiredAccounts.forEach(delete(account:))
    }

    func load() throws -> String? {
        try loadConfig()?.installationToken
    }
    ...
    func loadConfig() throws -> CredentialConfig? {
        ...
        guard let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let installationToken = object["installation_token"] as? String,
              ...
              let activationRevision = object["activation_revision"] as? NSNumber,
              activationRevision.int64Value > 0,
              ...
              representation == supportedRepresentationContract else { throw CoreError.keychain }
        return CredentialConfig(
            activation: ActivationIdentity(identity: SessionIdentity(
                origin: origin,
                namespace: namespace,
                representationContract: representation
            ), revision: activationRevision.int64Value),
...
struct BackgroundUploadClaim: Hashable {
    ...
    init(queueID: String, owner: String, identityRevision: Int64 = 1) {
...
final class AppGroupQueueRepository {
    ...
    func enqueueOrReuse(url: String, identity: QueueIdentity, now: Date = Date()) throws -> QueueEnqueueResult {
        ...
        do {
            let revision = try requireActiveRevisionUnlocked(for: identity)
            if let reusable = try findReusableUnlocked(url: url, identity: identity, revision: revision) {
                try execute("COMMIT;")
    ...
    func findReusable(url: String, identity: QueueIdentity) throws -> QueueEntry? {
        try validateQueueIdentity(identity)
        lock.lock(); defer { lock.unlock() }
        guard let active = try activeSessionSnapshotUnlocked(), active.queueIdentity == identity else { return nil }
        return try findReusableUnlocked(url: url, identity: identity, revision: active.revision)
    }

    private func findReusableUnlocked(url: String, identity: QueueIdentity, revision: Int64) throws -> QueueEntry? {
        let statement = try prepare("""
    ...
    @discardableResult
    func releaseClaim(entry: QueueEntry, owner: String, now: Date = Date()) throws -> Bool {
        lock.lock(); defer { lock.unlock() }
        let statement = try prepare("""
            UPDATE queue_entries SET lease_owner=NULL, lease_expires_at=NULL, updated_at=?
            WHERE id=? AND api_origin=? AND client_data_namespace=? AND identity_revision=?
              AND lease_owner=?
            """)
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, now.timeIntervalSince1970); bind(statement, 2, entry.id)
        bind(statement, 3, entry.identity.origin); bind(statement, 4, entry.identity.namespace)
        bind(statement, 5, entry.identityRevision); bind(statement, 6, owner)
        guard sqlite3_step(statement) == SQLITE_DONE else { throw CoreError.database }
        return sqlite3_changes(database) == 1
    }
    ...
    func reconcileBackgroundClaim(_ claim: BackgroundUploadClaim, now: Date = Date()) throws -> BackgroundClaimReconcileResult {
        ...
        guard claim.identityRevision == active.revision else { return .stale }
        ...
        guard identity == QueueIdentity(origin: active.identity.origin, namespace: active.identity.namespace),
              ...
              revision == claim.identityRevision,
    ...
    func list(identity: QueueIdentity? = nil) throws -> [QueueEntry] {
        ...
        var entries: [QueueEntry] = []
        while sqlite3_step(statement) == SQLITE_ROW {
            entries.append(makeEntry(statement, identity: identity, activeRevision: active?.revision))
        }
        return entries
    }
    ...
    func recent(identity: QueueIdentity? = nil) throws -> RecentResult? {
        ...
        if let identity, let active, identity == active.queueIdentity {
            ...
            if sqlite3_step(exact) == SQLITE_ROW {
                ...
                return makeRecent(exact, requestedIdentity: identity, activeRevision: active.revision)
            }
    ...
    private func activeSessionSnapshotUnlocked() throws -> ActiveSessionSnapshot? {
        let statement = try prepare("SELECT key, value FROM metadata WHERE key IN ('api_origin', 'client_data_namespace', 'representation_contract')")
        defer { sqlite3_finalize(statement) }
        var values: [String: String] = [:]
        while sqlite3_step(statement) == SQLITE_ROW { values[text(statement, 0)] = text(statement, 1) }
        ...
        let identity = SessionIdentity(origin: origin, namespace: namespace, representationContract: representation)
    ...
    @discardableResult
    func activate(session: SessionIdentity, now: Date = Date()) throws -> ActivationIdentity {
        ...
        do {
            ...
            try execute("COMMIT;")
            _ = now
            return ActivationIdentity(identity: session, revision: revision)
        } catch {
            try? execute("ROLLBACK;")
            throw error
        }
    ...
    /// Restores only the active pointer after a Keychain activation commit
    /// fails. The monotonic counter is deliberately never decremented or reused.
    func restoreActivation(_ activation: ActivationIdentity?) throws {
        lock.lock(); defer { lock.unlock() }
        try execute("BEGIN IMMEDIATE;")
        do {
            if let activation {
                try validateSessionIdentity(activation.identity)
                guard activation.revision > 0 else { throw CoreError.database }
                let statement = try prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES(?, ?)")
                defer { sqlite3_finalize(statement) }
                let values = [
                    ("api_origin", activation.identity.origin),
                    ("client_data_namespace", activation.identity.namespace),
                    ("representation_contract", activation.identity.representationContract),
                    ("active_identity_revision", String(activation.revision)),
                ]
                for (key, value) in values {
                    bind(statement, 1, key); bind(statement, 2, value)
                    guard sqlite3_step(statement) == SQLITE_DONE else { throw CoreError.database }
                    sqlite3_reset(statement); sqlite3_clear_bindings(statement)
                }
            } else {
                try execute("DELETE FROM metadata WHERE key IN ('api_origin', 'client_data_namespace', 'representation_contract', 'active_identity_revision')")
            }
            try execute("COMMIT;")
        } catch {
            try? execute("ROLLBACK;")
    ...
    func migrateIdentity(id: String, to target: QueueIdentity, now: Date = Date()) throws -> Bool {
        ...
        let oldRevision = sqlite3_column_int64(lookup, 2)
        try validateQueueIdentity(QueueIdentity(origin: oldOrigin, namespace: oldNamespace))
        let oldLeaseOwner = textOptional(lookup, 3)
        let oldLeaseExpiry = date(lookup, 4)
        guard oldOrigin != target.origin || oldNamespace != target.namespace || oldRevision != active.revision else { return false }
        ...
        bind(statement, 10, oldRevision)
        bind(statement, 11, now.timeIntervalSince1970)
        guard sqlite3_step(statement) == SQLITE_DONE else { throw CoreError.database }
        return sqlite3_changes(database) == 1
    }

    /// Migrates exactly one row from the revision the user confirmed to the
    /// revision that was active in that same confirmation. A later R2 -> R3
    /// activation cannot redirect an in-flight R1 -> R2 migration into R3.
    @discardableResult
    func migrateIdentity(
        ...
        try validateSessionIdentity(source.identity)
        try validateSessionIdentity(target.identity)
        guard source.revision >= 0,
              target.revision > 0,
        ...
        try execute("BEGIN IMMEDIATE;")
        do {
            guard try activeSessionSnapshotUnlocked() == target else {
                try execute("COMMIT;")
                return false
            }
            if try hasQueueMigrationReceiptUnlocked(id: id, source: source, target: target) {
                try execute("COMMIT;")
                return true
            }
            ...
            let storedIdentity = QueueIdentity(origin: text(lookup, 0), namespace: text(lookup, 1))
            let storedRevision = sqlite3_column_int64(lookup, 2)
            let leaseOwner = textOptional(lookup, 3)
            sqlite3_finalize(lookup)

            if storedIdentity == target.queueIdentity, storedRevision == target.revision {
                try insertQueueMigrationReceiptUnlocked(id: id, source: source, target: target, now: now)
                try execute("COMMIT;")
                return true
            }
            guard storedIdentity == source.queueIdentity,
                  storedRevision == source.revision,
                  leaseOwner == nil else {
                try execute("COMMIT;")
                return false
            }
            ...
            let statement = try prepare("""
                ...
                """)
            bind(statement, 1, UUID().uuidString.lowercased())
            bind(statement, 2, target.identity.origin)
            bind(statement, 3, target.identity.namespace)
            bind(statement, 4, target.revision)
            bind(statement, 5, QueueState.pendingSubmit.rawValue)
            bind(statement, 6, now.timeIntervalSince1970)
            bind(statement, 7, id)
            bind(statement, 8, source.identity.origin)
            bind(statement, 9, source.identity.namespace)
            bind(statement, 10, source.revision)
    ...
    /// Holds SQLite's writer reservation while a second durable store commits.
    /// Every activation uses the same database, so no scene or extension can
    /// replace the confirmed target until `perform` and this fence have committed.
    func withActiveTargetFence(_ target: ActivationIdentity, perform: () throws -> Bool) throws -> Bool {
        try validateSessionIdentity(target.identity)
        guard target.revision > 0 else { return false }
        lock.lock(); defer { lock.unlock() }
        try execute("BEGIN IMMEDIATE;")
        do {
            guard try activeSessionSnapshotUnlocked() == target else {
    ...
    func hasMigratableRecent(from source: ActivationIdentity, to target: ActivationIdentity) throws -> Bool {
        ...
        guard source.revision >= 0,
              ...
              source != target else { return false }
        lock.lock(); defer { lock.unlock() }
        guard try activeSessionSnapshotUnlocked() == target else { return false }
        let statement = try prepare("""
            SELECT 1 FROM recent_results
            WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?
              AND migrated_to_revision IS NULL
            """)
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, source.identity.origin)
        bind(statement, 2, source.identity.namespace)
        bind(statement, 3, source.revision)
        return sqlite3_step(statement) == SQLITE_ROW
    }

    func recentMigrationCandidates(to target: ActivationIdentity) throws -> [RecentMigrationCandidate] {
        try validateSessionIdentity(target.identity)
        guard target.revision > 0 else { return [] }
        lock.lock(); defer { lock.unlock() }
        guard try activeSessionSnapshotUnlocked() == target else { return [] }
        let statement = try prepare("""
            SELECT api_origin, client_data_namespace, identity_revision, created_at
            FROM recent_results
            WHERE api_origin=? AND client_data_namespace=? AND identity_revision<>?
              AND migrated_to_revision IS NULL
            ORDER BY created_at DESC, identity_revision DESC
            """)
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, target.identity.origin)
        bind(statement, 2, target.identity.namespace)
        bind(statement, 3, target.revision)
        var output: [RecentMigrationCandidate] = []
        while sqlite3_step(statement) == SQLITE_ROW {
            output.append(RecentMigrationCandidate(
                identity: QueueIdentity(origin: text(statement, 0), namespace: text(statement, 1)),
    ...
    /// Copies a frozen recent result into the active revision only after an
    /// explicit user decision. The source remains available as audit history;
    /// refresh transient state is intentionally not adopted.
    @discardableResult
    func migrateRecent(from source: ActivationIdentity, to target: QueueIdentity) throws -> Bool {
        try validateSessionIdentity(source.identity)
        try validateQueueIdentity(target)
        lock.lock(); defer { lock.unlock() }
        guard source.revision >= 0,
              let active = try activeSessionSnapshotUnlocked(),
              active.queueIdentity == target,
              active != source else { return false }
        return try migrateRecentUnlocked(from: source, to: active)
    }
    ...
    func migrateRecent(from source: ActivationIdentity, to target: ActivationIdentity) throws -> Bool {
        ...
        lock.lock(); defer { lock.unlock() }
        guard try activeSessionSnapshotUnlocked() == target else { return false }
        return try migrateRecentUnlocked(from: source, to: target)
    }
    ...
    func retryIdentityBlocked(identity: QueueIdentity, now: Date = Date()) throws -> Int {
        ...
        let statement = try prepare("""
            UPDATE queue_entries SET state=?, first_failed_at=NULL, next_attempt_at=NULL,
             ...
             WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?
             AND state IN ('blocked_auth', 'BLOCKED_AUTH')
             AND (lease_owner IS NULL OR lease_expires_at<=?)
            """)
        defer { sqlite3_finalize(statement) }
    ...
    func clearRecent() throws {
        lock.lock(); defer { lock.unlock() }
        try execute("DELETE FROM recent_results; DELETE FROM recent_result;")
    }
    ...
    func refreshCapture(identity: QueueIdentity) throws -> RefreshCommitCapture? {
        ...
        let statement = try prepare("SELECT link_id, api_origin, client_data_namespace, identity_revision FROM recent_results WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?")
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, identity.origin); bind(statement, 2, identity.namespace); bind(statement, 3, active.revision)
        guard sqlite3_step(statement) == SQLITE_ROW else { return nil }
    ...
    func recordRefreshBlocked(capture: RefreshCommitCapture, notBefore: Date?, reason: String) throws -> RefreshCommitResult {
        ...
        guard let active = try activeSessionSnapshotUnlocked(),
              QueueIdentity(origin: active.identity.origin, namespace: active.identity.namespace) == capture.identity,
              active.revision == capture.identityRevision else { return .identityChanged }
        let statement = try prepare("UPDATE recent_results SET refresh_not_before=?, refresh_block_reason=? WHERE api_origin=? AND client_data_namespace=? AND identity_revision=? AND link_id=?")
        defer { sqlite3_finalize(statement) }
    ...
    private func migrate() throws {
        try execute("""
            ...
            CREATE TABLE IF NOT EXISTS identity_migration_queue_receipts (
              queue_id TEXT NOT NULL,
              api_origin TEXT NOT NULL,
              client_data_namespace TEXT NOT NULL,
              source_revision INTEGER NOT NULL,
              target_revision INTEGER NOT NULL,
              completed_at REAL NOT NULL,
              PRIMARY KEY(queue_id, api_origin, client_data_namespace, source_revision, target_revision));
            """)
        ...
        try addColumnIfMissing(table: "recent_results", column: "migrated_to_revision", definition: "INTEGER")
        // Legacy revision 0 is copied only into the explicit quarantine
        ...
        try execute(
            "DELETE FROM metadata WHERE key NOT IN " +
                ...
                "'identity_revision_counter', 'active_identity_revision')"
        )
    }
    ...
    private func makeEntry(_ statement: OpaquePointer, identity: QueueIdentity?, activeRevision: Int64? = nil) -> QueueEntry {
        let entryIdentity = QueueIdentity(origin: text(statement, 5), namespace: text(statement, 6))
        let storedRevision = sqlite3_column_int64(statement, 7)
        let identityMismatch = identity.map { $0 != entryIdentity || activeRevision != storedRevision } ?? false
        return QueueEntry(
            ...
            identityRevision: storedRevision,
    ...
    /// crash observes the receipt and cannot replace a newer target recent row.
    private func migrateRecentUnlocked(from source: ActivationIdentity, to target: ActivationIdentity) throws -> Bool {
        try execute("BEGIN IMMEDIATE;")
        do {
            guard try activeSessionSnapshotUnlocked() == target else {
                try execute("COMMIT;")
            ...
            let lookup = try prepare("""
                SELECT migrated_to_revision FROM recent_results
                WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?
                """)
            bind(lookup, 1, source.identity.origin)
            bind(lookup, 2, source.identity.namespace)
            bind(lookup, 3, source.revision)
            guard sqlite3_step(lookup) == SQLITE_ROW else {
                sqlite3_finalize(lookup)
                try execute("COMMIT;")
                return false
            }
            let migratedRevision: Int64? = sqlite3_column_type(lookup, 0) == SQLITE_NULL
                ? nil
                : sqlite3_column_int64(lookup, 0)
            sqlite3_finalize(lookup)
            if migratedRevision == target.revision {
                try execute("COMMIT;")
                return true
            }
            guard migratedRevision == nil else {
                try execute("COMMIT;")
                return false
            }
            ...
            let copy = try prepare("""
                ...
                WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?
                  AND migrated_to_revision IS NULL
                """)
            bind(copy, 1, target.identity.origin)
            bind(copy, 2, target.identity.namespace)
            bind(copy, 3, target.revision)
            bind(copy, 4, source.identity.origin)
            bind(copy, 5, source.identity.namespace)
            bind(copy, 6, source.revision)
            guard sqlite3_step(copy) == SQLITE_DONE, sqlite3_changes(database) == 1 else {
            ...
            let receipt = try prepare("""
                UPDATE recent_results SET migrated_to_revision=?
                WHERE api_origin=? AND client_data_namespace=? AND identity_revision=?
                  AND migrated_to_revision IS NULL
                """)
            bind(receipt, 1, target.revision)
            bind(receipt, 2, source.identity.origin)
            bind(receipt, 3, source.identity.namespace)
            bind(receipt, 4, source.revision)
            guard sqlite3_step(receipt) == SQLITE_DONE, sqlite3_changes(database) == 1 else {
    ...
    private func hasQueueMigrationReceiptUnlocked(
        id: String,
        source: ActivationIdentity,
        target: ActivationIdentity
    ) throws -> Bool {
        let statement = try prepare("""
            SELECT 1 FROM identity_migration_queue_receipts
            WHERE queue_id=? AND api_origin=? AND client_data_namespace=?
              AND source_revision=? AND target_revision=?
            """)
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, id)
        bind(statement, 2, source.identity.origin)
        bind(statement, 3, source.identity.namespace)
        bind(statement, 4, source.revision)
        bind(statement, 5, target.revision)
        return sqlite3_step(statement) == SQLITE_ROW
    }

    private func insertQueueMigrationReceiptUnlocked(
        id: String,
        source: ActivationIdentity,
        target: ActivationIdentity,
        now: Date
        ...
        let statement = try prepare("""
            INSERT OR IGNORE INTO identity_migration_queue_receipts
              (queue_id, api_origin, client_data_namespace, source_revision, target_revision, completed_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """)
        defer { sqlite3_finalize(statement) }
        bind(statement, 1, id)
...
final class AppGroupQueueRepository {
    ...
    func releaseClaim(entry: QueueEntry, owner: String, now: Date = Date()) throws -> Bool { throw CoreError.database }
    ...
    func activate(session: SessionIdentity, now: Date = Date()) throws -> ActivationIdentity { throw CoreError.database }
    func restoreActivation(_ activation: ActivationIdentity?) throws { throw CoreError.database }
    @discardableResult
    ...
    func migrateIdentity(id: String, from source: ActivationIdentity, to target: ActivationIdentity, now: Date = Date()) throws -> Bool { throw CoreError.database }
    func withActiveTargetFence(_ target: ActivationIdentity, perform: () throws -> Bool) throws -> Bool { throw CoreError.database }
    func hasMigratableRecent(from source: ActivationIdentity, to target: ActivationIdentity) throws -> Bool { throw CoreError.database }
    func recentMigrationCandidates(to target: ActivationIdentity) throws -> [RecentMigrationCandidate] { throw CoreError.database }
    @discardableResult
    func migrateRecent(from source: ActivationIdentity, to target: QueueIdentity) throws -> Bool { throw CoreError.database }
    @discardableResult
    func migrateRecent(from source: ActivationIdentity, to target: ActivationIdentity) throws -> Bool { throw CoreError.database }
    @discardableResult
...
final class WebTagAPIClient: NSObject, URLSessionTaskDelegate {
    ...
    func validateSession(origin: String, installationToken: String) async -> Result<SessionIdentity, ClassifiedFailure> {
        guard !installationToken.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
        ...
        return await request(origin: normalizedOrigin, installationToken: installationToken, method: "GET", path: "/api/session", body: nil, idempotencyKey: nil, acceptedStatusCodes: [200]) { data, namespace in
            guard let object = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            guard let representation = object["representation_contract"] as? String,
                  ...
                  representation == supportedRepresentationContract else { throw CoreError.invalidSession }
            let identity = SessionIdentity(origin: normalizedOrigin, namespace: bodyNamespace, representationContract: representation)
            try validateSessionIdentity(identity)
            return identity
        }
    }

    func submit(identity: SessionIdentity, installationToken: String, url: String, idempotencyKey: String) async -> Result<SubmitResponse, ClassifiedFailure> {
        ...
        return await request(origin: normalizedOrigin, installationToken: installationToken, method: "POST", path: "/api/links", body: body, idempotencyKey: idempotencyKey, acceptedStatusCodes: [202]) { data, namespace in
            guard namespace == identity.namespace else { throw CoreError.identityMismatch }
            return try Self.decodeSubmitPayload(data)
        }
    }

    func refresh(identity: SessionIdentity, installationToken: String, linkID: String) async -> Result<SubmitResponse, ClassifiedFailure> {
        guard (try? validateSessionIdentity(identity)) != nil else {
            return .failure(ClassifiedFailure(category: .invalidClientResponse, status: nil, errorCode: nil, retryAfter: nil))
        }
        guard identity.isSupported,
        ...
        return await request(origin: normalizedOrigin, installationToken: installationToken, method: "POST", path: "/api/links/\(linkID)/refresh", body: nil, idempotencyKey: nil, acceptedStatusCodes: [202]) { data, namespace in
            guard namespace == identity.namespace else { throw CoreError.identityMismatch }
    ...
    func decodeBackgroundSubmit(data: Data, response: HTTPURLResponse?, error: Error?, expectedNamespace: String) -> Result<SubmitResponse, ClassifiedFailure> {
        ...
        do {
            // A complete, identity-bound acknowledgement is stronger evidence
            // than a transport error delivered after the response body.
            return .success(try Self.decodeSubmitPayload(data))
        } catch {
    ...
    private func request<T>(origin: String, installationToken: String, method: String, path: String, body: Data?, idempotencyKey: String?, acceptedStatusCodes: Set<Int>, decode: (Data, String?) throws -> T) async -> Result<T, ClassifiedFailure> {
...
struct BackgroundUploadResult {
    ...
    init(queueID: String, owner: String, identityRevision: Int64 = 1, data: Data, response: HTTPURLResponse?, error: Error?) {
        self.queueID = queueID
        self.owner = owner
        self.identityRevision = identityRevision
        self.data = data
...
final class BackgroundUploadSessionController: NSObject, URLSessionDataDelegate, URLSessionTaskDelegate, QueueUploadScheduling {
    ...
    private struct TaskDescriptor {
        ...
        static func encode(queueID: String, owner: String, identityRevision: Int64) -> String {
            "\(queueID)|\(owner)|\(identityRevision)"
        }
        ...
        static func decode(_ value: String?) -> TaskDescriptor? {
            ...
            guard parts.count == 3,
                  let canonicalQueueID = UUID(uuidString: parts[0])?.uuidString.lowercased(),
                  !parts[1].isEmpty,
                  let revision = Int64(parts[2]), revision > 0 else { return nil }
            return TaskDescriptor(queueID: canonicalQueueID, owner: parts[1], identityRevision: revision)
        }
    ...
    func schedule(entry: QueueEntry, identity: SessionIdentity, installationToken: String, owner: String) throws {
        ...
        let claim = BackgroundUploadClaim(queueID: entry.id, owner: owner, identityRevision: entry.identityRevision)
        ...
        task.taskDescription = TaskDescriptor.encode(queueID: entry.id, owner: owner, identityRevision: entry.identityRevision)
        task.resume()
    }
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Set<BackgroundUploadClaim>, Never>) in
            session.getAllTasks { tasks in
                continuation.resume(returning: Set(tasks.compactMap { task in
                    ...
                    return BackgroundUploadClaim(queueID: descriptor.queueID, owner: descriptor.owner, identityRevision: descriptor.identityRevision)
                }))
            }
        }
    }
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Set<BackgroundUploadClaim>, Never>) in
            session.getAllTasks { tasks in
                ...
                for task in tasks {
                    ...
                    let claim = BackgroundUploadClaim(queueID: descriptor.queueID, owner: descriptor.owner, identityRevision: descriptor.identityRevision)
    ...
    func cleanupOrphanedUploadFiles(retaining claims: Set<BackgroundUploadClaim>) {
        ...
        for file in files where file.pathExtension == "json" {
            ...
            guard parts.count == 3,
                  let queueID = UUID(uuidString: parts[0])?.uuidString.lowercased(),
                  let revision = Int64(parts[2]), revision > 0 else {
                try? FileManager.default.removeItem(at: file)
                continue
            }
            let claim = BackgroundUploadClaim(queueID: queueID, owner: parts[1], identityRevision: revision)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        stateLock.unlock()
        if let descriptor { removeUploadFile(claim: BackgroundUploadClaim(queueID: descriptor.queueID, owner: descriptor.owner, identityRevision: descriptor.identityRevision)) }
        guard let descriptor, let queueID, let handler else {
            finishTaskProcessing()
            return
        }
        let result = BackgroundUploadResult(queueID: queueID, owner: descriptor.owner, identityRevision: descriptor.identityRevision, data: data, response: task.response as? HTTPURLResponse, error: error)
        Task {
    ...
    private func uploadFileURL(directory: URL, claim: BackgroundUploadClaim) -> URL {
        directory.appendingPathComponent("\(claim.queueID)|\(claim.owner)|\(claim.identityRevision).json")
    }
...
extension Notification.Name {
    static let webTagQueueWakeDue = Notification.Name("WebTagQueueWakeDue")
}
...
protocol DurableQueueWakeAdapting: AnyObject {
    func replace(deadline: Date?) throws
}

enum DurableQueueWakeError: Error {
    case unavailable
}

#if canImport(BackgroundTasks)
final class BackgroundTasksQueueWakeAdapter: DurableQueueWakeAdapting {
    static let shared = BackgroundTasksQueueWakeAdapter(identifier: AppIdentifiers.backgroundWakeIdentifier)
    ...
    func register(handler: @escaping (BGAppRefreshTask) -> Void) -> Bool {
        ...
        return BGTaskScheduler.shared.register(forTaskWithIdentifier: identifier, using: nil) { task in
            self.lock.lock(); self.scheduledDeadline = nil; self.lock.unlock()
    ...
    func replace(deadline: Date?) throws {
        guard !identifier.isEmpty else { throw DurableQueueWakeError.unavailable }
        lock.lock()
        if scheduledDeadline == deadline {
            lock.unlock()
            return
        }
        ...
        do {
            ...
            lock.lock(); scheduledDeadline = nil; lock.unlock()
            throw error
        }
    }
}
#else
final class BackgroundTasksQueueWakeAdapter: DurableQueueWakeAdapting {
    static let shared = BackgroundTasksQueueWakeAdapter()
    private init() {}
    func replace(deadline: Date?) throws { throw DurableQueueWakeError.unavailable }
}
#endif

/// One production owner fans the same authoritative deadline out to a durable
/// system request and an optional low-latency foreground timer.
final class DurableQueueWakeScheduler: QueueWakeScheduling {
    static let shared = DurableQueueWakeScheduler(
        durable: BackgroundTasksQueueWakeAdapter.shared,
        foreground: InProcessQueueWakeScheduler {
            NotificationCenter.default.post(name: .webTagQueueWakeDue, object: nil)
        }
    )

    private let durable: DurableQueueWakeAdapting
    private let foreground: InProcessQueueWakeScheduler?
    private let lock = NSLock()
    private var lastDurableDeadline: Date?
    private var hasScheduled = false

    init(durable: DurableQueueWakeAdapting, foreground: InProcessQueueWakeScheduler? = nil) {
        self.durable = durable
        self.foreground = foreground
    }

    func setOnWake(_ handler: @escaping () -> Void) {
        foreground?.setOnWake(handler)
    }

    func markDurableWakeDelivered() {
        lock.lock(); hasScheduled = false; lock.unlock()
    }

    func schedule(deadline: Date?) {
        foreground?.schedule(deadline: deadline)
        lock.lock()
        if hasScheduled, lastDurableDeadline == deadline {
            lock.unlock()
            return
        }
        hasScheduled = true
        lastDurableDeadline = deadline
        lock.unlock()
        do {
            try durable.replace(deadline: deadline)
        } catch {
            lock.lock(); hasScheduled = false; lock.unlock()
        }
    }
...
final class ShareSubmissionCoordinator {
    ...
    init(repository: AppGroupQueueRepository, credentials: CredentialConfigLoading = KeychainCredentialStore(), api: WebTagAPIClient = WebTagAPIClient(), background: QueueUploadScheduling? = nil, clock: ShareMonotonicClock = SystemMonotonicClock(), wakeScheduler: QueueWakeScheduling) {
    ...
    func submit(url: String, identity: SessionIdentity, interactionDeadline: TimeInterval? = nil, now: Date = Date()) async -> SubmissionOutcome {
        guard identity.isSupported else { return .configurationRequired }
        ...
        do {
            guard let active = try repository.activeSessionSnapshot(),
                  ...
                  storedConfiguration.activation == active else { return .configurationRequired }
        } catch {
            return .configurationRequired
        }
        let installationToken = storedConfiguration.installationToken
        do {
            ...
            guard let result = await withinInteractionDeadline(interactionDeadline, work: { [api] in
                await api.submit(identity: identity, installationToken: installationToken, url: url, idempotencyKey: claimed.idempotencyKey)
            }) else {
                ...
                if let background {
                    do {
                        try background.schedule(entry: claimed, identity: identity, installationToken: installationToken, owner: owner)
    ...
    func reconcileAndDrain(excluding: Set<BackgroundUploadClaim> = [], now: Date = Date(), maxItems: Int = 16, shouldContinue: () -> Bool = { true }) async {
        try? repository.releaseExpiredLeases(now: now)
        var processed = 0
        while processed < max(0, maxItems), shouldContinue(), await drainOne(
            excluding: excluding,
            now: Date(),
    ...
    func drainOne(
        excluding: Set<BackgroundUploadClaim> = [],
        now: Date = Date(),
        ...
        let installationToken: String
        do {
            guard let active = try repository.activeSessionSnapshot(), active.identity.isSupported,
                  let storedConfiguration = try credentials.loadConfig(),
                  storedConfiguration.activation == active else { return false }
            identity = active.identity
            installationToken = storedConfiguration.installationToken
        } catch { return false }
        guard let candidate = (try? repository.due(now: now))?.first(where: {
            !excluding.contains(BackgroundUploadClaim(queueID: $0.id, owner: $0.leaseOwner ?? "", identityRevision: $0.identityRevision))
        }) else { return false }
        let owner = UUID().uuidString
        ...
        let result = await api.submit(identity: identity, installationToken: installationToken, url: entry.url, idempotencyKey: entry.idempotencyKey)
        guard shouldCommit(), !Task.isCancelled else {
            _ = try? repository.releaseClaim(entry: entry, owner: owner, now: Date())
            return true
        }
    ...
    func handleBackgroundCompletion(_ result: BackgroundUploadResult, now: Date = Date()) async {
        guard let original = try? repository.entry(id: result.queueID),
              ...
              original.identityRevision == result.identityRevision,
...
enum SettingsQueueGroup: String, CaseIterable {
    ...
    static func group(for state: QueueState) -> SettingsQueueGroup {
        ...
        case .blockedAuth: return .blockedCredential
...
///
/// There is deliberately no origin or installation token here. A lifecycle reload replaces
