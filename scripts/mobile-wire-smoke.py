...
network, real credentials, or a deployed WebTag instance: session identity,
...
class FakeAPI(BaseHTTPRequestHandler):
    ...
    def authorized(self) -> bool:
        if self.headers.get("Authorization") == f"Bearer {INSTALLATION_TOKEN}":
            return True
        self.send_json(401, {"error": {"error_code": "invalid_token"}})
        return False
...
def main() -> int:
    ...
    try:
        ...
        assert session["representation_contract"] == "v3"
        assert set(session) == {"client_data_namespace", "representation_contract"}
        ...
        print("mobile wire smoke passed: installation session, minimal submit, idempotent replay, and refresh")
        return 0
    finally:
        server.shutdown()
        server.server_close()
