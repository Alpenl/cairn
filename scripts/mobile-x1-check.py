...
RETIRED_QUEUE_STATES = {"blocked_scope", "blocked_quota"}
...
def check_queue_and_retry_contract(gate: Gate) -> None:
    ...
    for state in RETIRED_QUEUE_STATES:
        gate.require(
            f'"{state}"' not in android_contract,
            f"Android QueueState still exposes retired state {state}",
        )
        gate.require(
            state not in android_queue and state.upper() not in android_queue,
            f"Android Room contract still exposes retired state {state}",
        )
        gate.require(
            state not in ios_core,
            f"iOS QueueState still exposes retired state {state}",
        )
...
def check_ci_dispatch(gate: Gate) -> None:
    ...
    This used to be guaranteed by a ``pull_request.paths`` list inside each
    mobile workflow. Those standalone triggers were removed so that a pull
    ...
    gate.require(
        re.search(r"^\s*needs:\s*\[[^]]*\bios\b", ci_workflow, flags=re.MULTILINE)
        is not None,
        f"{label} gate must depend on the ios job",
    )
...
def check_workflows(gate: Gate) -> None:
    ...
    label = str(ANDROID_WORKFLOW.relative_to(REPOSITORY))
    gate.require("workflow_call:" in android_workflow, f"{label} must support reusable workflow calls")
    gate.require("scripts/mobile-x1-check.py" in android_workflow, f"{label} must run the X1 gate")
    gate.require("scripts/mobile-wire-smoke.py" in android_workflow, f"{label} must run the local wire smoke")
    gate.require("mobile/shared/fixtures/validate.py" in android_workflow, f"{label} must validate fixture shape")
    gate.require("mobile/shared/fixtures/compare.py" in android_workflow, f"{label} must compare fixture results")
    gate.require("secrets." not in android_workflow, f"{label} must not read release secrets")
    gate.require("environment:" not in android_workflow, f"{label} must not attach a release environment")
    gate.require("git diff --check" in android_workflow, f"{label} must check whitespace")
    for pattern in ("*.p12", "*.mobileprovision", "*.jks", "*.keystore"):
        gate.require(
            pattern in android_workflow,
    ...
    for line in ios_workflow.splitlines():
        ...
        gate.require(
            re.search(r"@[0-9a-fA-F]{40}$", action) is not None,
            f".github/workflows/mobile-ios.yml action is not pinned to a commit: {action}",
        )
    for forbidden in (
        "pnpm install",
        "pnpm audit",
    ...
    for forbidden in ("assembleRelease", "sdkmanager", "avdmanager"):
        gate.require(
            forbidden not in android_workflow,
    ...
    for forbidden in ("secrets.", "environment:", "archive", "exportArchive"):
        gate.require(
            forbidden not in ios_workflow,
            f"iOS verification workflow must not contain {forbidden}",
        )
...
def main() -> int:
    ...
    if gate.errors:
        ...
        return 1
    print(
        "mobile X1 static gate passed: fixture wiring, six queue states, "
        "retry/identity/refresh contracts, storage/network security, "
        "signing-file scans, and pinned Android/iOS verification workflows"
    )
    return 0
