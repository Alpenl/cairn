...
#
# It never writes a credential. /etc/cairn-updater.env arrives with an empty
# DEPLOY_AUTH_TOKEN, the helper refuses to start with an empty token, and the
# final report tells the operator what is left to do by hand.
set -euo pipefail
IFS=$'\n\t'
...
ensure_core_env() {
	if [[ ! -e $CORE_ENV ]]; then
		...
		chmod 0640 "$CORE_ENV"
		note "created $CORE_ENV as an empty template"
		manual "Fill in $CORE_ENV (DATABASE_URL, AI_*, CURSOR_SIGNING_KEY); webtag.service will not start until you do."
	else
		# Existing content is never touched, only its ownership and mode.
		chown "root:$SERVICE_GROUP" "$CORE_ENV"
		chmod 0640 "$CORE_ENV"
	fi
}
